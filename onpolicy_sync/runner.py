...
class OnPolicyRunner(object):
    ...
    def start_train(self, checkpoint: Optional[str] = None):
        ...
        if nworkers > 1:
            ...
            distributed_barrier = self.mp_ctx.Barrier(nworkers, timeout=10000)
    ...
    def log(self, nworkers, checkpoints=None):
        ...
        try:
            ...
            while True:
                try:
                    package = self.queues["results"].get(timeout=10000)  # TODO wait for 10 seconds, then check all workers are alive
                except queue.Empty:
