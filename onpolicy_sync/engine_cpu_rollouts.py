...
from onpolicy_sync.storage_device import RolloutStorage
...
class OnPolicyRLEngine(object):
    ...
    def update(self, rollouts: RolloutStorage) -> None:
        ...
        for e in range(self.update_epochs):
            data_generator = rollouts.recurrent_generator(
                advantages, self.update_mini_batches, device=self.device
            )
    ...
    def collect_rollout_step(self, rollouts: RolloutStorage, render=None):
        # sample actions
        with torch.no_grad():
            step_observation = rollouts.pick_observation_step(rollouts.step, self.device)
    ...
    def remove_paused(self, observations):
        ...
        batch = batch_observations(running, device=torch.device("cpu"))
        ...
        while self.rollout_count < self.num_rollouts:
            ...
            with torch.no_grad():
                actor_critic_output, _ = self.actor_critic(
                    rollouts.pick_observation_step(-1, device=self.device),
