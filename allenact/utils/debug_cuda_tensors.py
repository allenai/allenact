...
def find_cuda_tensors(
    obj: Any, path: str = "root", visited: Set[int] = None, max_depth: int = 10
) -> List[str]:
    """
    ...
    try:
        ...
        if isinstance(obj, torch.Tensor) and obj.is_cuda:
            cuda_paths.append(
                f"{path} (shape: {obj.shape}, device: {obj.device}, dtype: {obj.dtype})"
            )
            return cuda_paths
        ...
        if isinstance(obj, dict):
            ...
            for attr_name in dir(obj):
                ...
                if (
                    ...
                    or attr_name in ["__class__", "__dict__", "__weakref__"]
...
def check_specific_objects_for_cuda(sampler_args: Dict[str, Any]) -> None:
    ...
    high_priority_checks = [
        ...
        (
            "action_hook_runner",
            sampler_args.get("controller_args", {}).get("action_hook_runner"),
        ),
    ]
...
def debug_cuda_in_task_sampler():
    ...
    print(
        "In debug_cuda_in_task_sampler",
        sorted(list(locals_dict.keys())),
        file=sys.stderr,
    )
...
if __name__ == "__main__":
    ...
    sampler_args = {
        ...
        "sensors": [],
        "some_tensor": (
            torch.randn(3, 3).cuda() if torch.cuda.is_available() else torch.randn(3, 3)
        ),
        "nested": {
            "data": (
                torch.zeros(10).cuda() if torch.cuda.is_available() else torch.zeros(10)
            )
        },
    }
