...
class ObjectNavTheRobotProjectTaskSampler(ObjectNavTaskSampler):
    def next_task(self) -> Optional[ObjectNavTask]:
        if self.max_tasks is not None and self.max_tasks <= 0:
            return None
        ...
        if self.env is not None:
            if scene != self.env.scene_name:
                self.env.reset(scene, renderClassImage=True, renderObjectImage=True)
        else:
            self.env = self._create_environment()
            self.env.start(
                scene_name=scene, renderClassImage=True, renderObjectImage=True
            )
