...
from models.resnet_tensor_object_nav_models import ResnetTensorObjectNavActorCriticMemory
...
class ObjectNav_RoboThor_RGB_DDPPO_Memory_ExperimentConfig(ExperimentConfig):
    ...
    @classmethod
    def tag(cls):
        return "ObjectNavRobothorRGBDDPPO_Memory"
    ...
    def machine_params(self, mode="train", **kwargs):
        if mode == "train":
            gpu_ids = [] if not torch.cuda.is_available() else [0, 1, 2, 3, 4, 5, 6]  # TODO vs4 only has 7 gpus
            nprocesses = 1 if not torch.cuda.is_available() else self.split_num_processes(len(gpu_ids))
        elif mode == "valid":
            nprocesses = 0
            gpu_ids = [] if not torch.cuda.is_available() else [6]  # TODO vs4 only has 7 GPUs
        elif mode == "test":
            nprocesses = 7  # each checkpoint 1 GPU - TODO
            gpu_ids = [] if not torch.cuda.is_available() else [0, 1, 2, 3, 4, 5, 6]  # TODO vs4 only has 7 gpus
        else:
    ...
    def create_model(cls, **kwargs) -> nn.Module:
        return ResnetTensorObjectNavActorCriticMemory(
